...
public class AuthrozieTranscation {
								...
								if (response != null) {
									...
									if (response.getMessages().getResultCode() == MessageTypeEnum.OK) {
										...
										if (result.getMessages() != null) {
											...
											String lastid = _AuthrozieTranscation.saveDataTranscationDetails(
	...
	public String saveDataTranscationDetails(int CompanyId, int CustomerId, String TransactionDetail, double SendingAmount,
		...
		try {
			...
			//String  randomCode  = UUID.randomUUID().toString();
			 String uniqueTxnID = UUID.randomUUID().toString().substring(0, 5);
			uniqueTxnID="txn_"+uniqueTxnID+"_"+PaymentGatewayTransactionId;
			String sInsertStatement = "INSERT INTO transactiondetails( CompanyId, CustomerId, TransactionDetail,SendingAmount,Charges,Fees,Tax, TransactionDate, Status,PaymentGatewayResponse,PaymentGatewayTransactionId,SendingCurrencyId,ReceivingAmount,ReceivingCurrencytId,BeneficiaryId,PaymentMethodId, DestinationCountryId, SourceCountryId,IsLive,TransferPurpose,ExchangeRate,DeliveryType,CreatedDate,TransactionId)";
			...
			_PreparedStatement.setString(24, uniqueTxnID);
			_PreparedStatement.executeUpdate();
			...
			//	String lastid = _ResultSetld.getString("TransactionId");
				ResultId = uniqueTxnID;
			//}
		} catch (Exception e) {
		...
		try {
...

			{
				...
				while (_ResultSet.next()) {
					...
					_AuthrozieTranscation.setTransactionId(_ResultSet.getString("TransactionId"));
	...
	public static ArrayList<AuthrozieTranscation> getAuthrozieTranscationById(String TransactionId) {
		Connection _Connection = MYSQLConnection.GetConnection();
	...
	private AuthrozieTranscation clear(AuthrozieTranscation _AuthrozieTranscation) {
		...
		_AuthrozieTranscation.setTransactionId("");
		return _AuthrozieTranscation;
	}
