...
public class PaymentFees {
			...
			if (_Connection != null) {
				...
				if (_Country.next()) {
					...
					if (_destinationCountry.next()) {
						ResultSet _feesCategoryId = _MYSQLHelper.GetResultSet(
								"SELECT * FROM feescategory where FeesCategoryId='" + _PaymentFees.FeesCategoryId + "'",
								_Connection);
						if (_feesCategoryId.next()) {
							...
							if (_agentId.next()) {
								...
								if (_PaymentFees.PaymentFessId <= 0) {
									 ...
									 * _PaymentFees.CompanyId +
									 * "' and PayInAgentId='" +
									 * _PaymentFees.PayInAgentId +
									 * "' and PayOutAgentId='" +
									 * _PaymentFees.PayOutAgentId +
									 * "' and FeesType='" +
									 * _PaymentFees.FeesType +
									 * "' and PaymentMethod='" +
									 * _PaymentFees.PaymentMethodId + "'",
									 * _Connection);
									 */

									ResultSet _ResultSetFees = _MYSQLHelper.GetResultSet("SELECT * FROM `paymentfees`"
											+ " WHERE ((`StartingAmount` <= '" + _PaymentFees.StartingAmount
											+ "' and EndAmount >='" + _PaymentFees.EndAmount
											+ "') or (EndAmount BETWEEN '" + _PaymentFees.StartingAmount + "' and  '"
											+ _PaymentFees.EndAmount + "'  or StartingAmount BETWEEN '"
											+ _PaymentFees.StartingAmount + "' and  '" + _PaymentFees.EndAmount
											+ "' and `StartingAmount` <= '" + _PaymentFees.StartingAmount
											+ "' and EndAmount >='" + _PaymentFees.EndAmount
											+ "')) and SourceCountry ='" + _PaymentFees.SourceCountry
											+ "' and DestinationCountry='" + _PaymentFees.DestinationCountry
											+ "' and FeesCategoryId='" + _PaymentFees.FeesCategoryId
											+ "' and CompanyId='" + _PaymentFees.CompanyId + "' and FeesType='"
											+ _PaymentFees.FeesType + "' and PaymentMethodId='"
											+ _PaymentFees.PaymentMethodId + "' and IsDeleted=0", _Connection);
									if (!_ResultSetFees.next()) {

										if (_PaymentFees.DestinationCountry == 242) {

											ResultSet _eu = _MYSQLHelper.GetResultSet(
													"SELECT * FROM country where iseuropeancountry=1", _Connection);
											while (_eu.next()) {
												...
												_PreparedStatement.setInt(4, _eu.getInt("country_id"));
										...
										_PaymentFees.setError("Fees already exists under Code " + _Exitscode
												+ " please edit Fee Code " + _Exitscode + " for new requirements");
										clear(_PaymentFees);
									}
									 ...
									 * 
									 * String _Exitscode =
									 * _ResultSetFeesCheck.getString("Code");
									 * 
									 * _PaymentFees.setResult("Failed");
									 * _PaymentFees.
									 * setError("Fees already exists under Code "
									 * +_Exitscode +" please edit Fee Code "+
									 * _Exitscode +" for new requirements");
									 * clear(_PaymentFees); }
									 * 
									 */
									...
									if (_PaymentFess.next()) {
										if (_PaymentFees.DestinationCountry == 242) {
											...
											while (_eu.next()) {
												...
												_PreparedStatement.setBoolean(14, _PaymentFees.ChargeSendingAmount);
												_PreparedStatement.setString(15, _PaymentFees.CreatedDate);
												_PreparedStatement.setInt(16, _eu.getInt("country_id"));
												_PreparedStatement.executeUpdate();
												_PaymentFees.setResult("Sucess");
	...
	public static ArrayList<PaymentFees> getPaymentFeesDetailsByCompany(int CompanyId, int FeesCategoryId) {
		Connection _Connection = MYSQLConnection.GetConnection();
				...
				if (CompanyId == 0) {
					...
					ResultSet _ResultSet = _MYSQLHelper
							.GetResultSet("SELECT * FROM paymentfees where IsDeleted=0 and CompanyId='" + CompanyId
									+ "' and FeesCategoryId='" + FeesCategoryId + "'", _Connection);
	...
	public PaymentFees getPaymentFeesnewdetails(PaymentFees _PaymentFees) {
		...
		try {
			if (_Connection != null) {
				...
				ResultSet _ResultSet = _MYSQLHelper
						.GetResultSet(
								"SELECT * FROM paymentfees where  IsDeleted=0 and PaymentMethodId='"
										+ _PaymentFees.PaymentMethodId + "'and SourceCountry='"
										+ _PaymentFees.SourceCountry + "' and DestinationCountry='"
										+ _PaymentFees.DestinationCountry + "' order by PaymentFessId desc",
								_Connection);
				if (_ResultSet.next()) {
	...
	public static ArrayList<PaymentFees> getPaymentFeesDetailsByPaymentMethod(int PaymentMethodId, int FeesCategoryId) {
		...
		try {
...

			{
				...
				ResultSet _ResultSet = _MYSQLHelper
						.GetResultSet("SELECT * FROM paymentfees where IsDeleted=0 and PaymentMethodId='"
								+ PaymentMethodId + "'and FeesCategoryId='" + FeesCategoryId + "'", _Connection);
