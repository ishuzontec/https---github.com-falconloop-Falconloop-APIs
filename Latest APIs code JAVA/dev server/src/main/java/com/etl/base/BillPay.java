...
public class BillPay {
	...
	private void setBillPayId(String BillPayId) {
		this.BillPayId = BillPayId;
	}

	private String getBillPayId() {
		return BillPayId;
	}
														...
														if (_tNodeList.getLength() > 0) {
															...
															if (_tNode.getNodeType() == Node.ELEMENT_NODE) {
																...
																if (_tElement.getNodeType() == Node.ELEMENT_NODE) {
																	...
																	String lastid = _AuthrozieTranscation
																	...
																	String Resultlastid = _BillPay.addbillPayDetails(
						...
						try {
							...
							try {
								...
								if (_MagicPay.Result == "Success") {
									...
									String Resultlastid = _BillPay.addbillPayDetails(_BillPay.CompanyId,
	...
	public String addbillPayDetails(int CompanyId, int CustomerId, String TransactionId, String SenderName,
		...
		try {
			...
			if (_ResultSetbillpayld.next()) {
				String lastid = _ResultSetbillpayld.getString("BillPayId");
				Result = lastid;
		...
		try {
			if (_Connection != null) {
				...
				while (_ResultSet.next()) {
					...
					_BillPay.setBillPayId(_ResultSet.getString("BillPayId"));
					_BillPay.setCompanyId(_ResultSet.getInt("CompanyId"));
					_BillPay.setCustomerId(_ResultSet.getInt("CustomerId"));
					_BillPay.setTransactionId(_ResultSet.getString("TransactionId"));
				...
				if (_tNodeList.getLength() > 0) {
					...
					if (_tNode.getNodeType() == Node.ELEMENT_NODE) {
						...
						if (resultpay == 200) {
							...
							_PreparedStatement.setString(2, _BillPay.BillPayId);
							_PreparedStatement.executeUpdate();
							_BillPay.setResult("Success");
						}
					}
