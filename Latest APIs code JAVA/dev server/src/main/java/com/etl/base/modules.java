...
public class modules {
	...
	public String Error;
	public String MultipleModuelId;
	
	
	
	private void setMultipleModuelId(String MultipleModuelId) {
		this.MultipleModuelId = MultipleModuelId;
	}

	private String getMultipleModuelId() {
		return MultipleModuelId;
	}
		...
		try {
			...
			if (_Connection != null) {
				...
				ResultSet _ResultSet = _MYSQLHelper
						.GetResultSet("SELECT * FROM modules where moduelId='" + moduelId + "'", _Connection);
				if (_ResultSet.next()) {
					_modules.setModuelId(_ResultSet.getInt("moduelId"));
					_modules.setModuelName(_ResultSet.getString("moduelName"));
					_modules.setModuelDescription(_ResultSet.getString("moduelDesc"));
	...
	public static ArrayList<modules> getmodulesDetailByIds(String ModulesId) {
			...
			if (_Connection != null) {
				...
				for (int i = 0; i < arrOfStrModules.length; i++) {
					...
					if (_ResultSetModule.next()) {
						...
						modules _modules = new modules();
						_modules.setModuelId(_ResultSetModule.getInt("moduelId"));
						_modules.setModuelName(_ResultSetModule.getString("moduelName"));
						_modules.setModuelDescription(_ResultSetModule.getString("moduelDesc"));
						_modules.setCreatedDate(_ResultSetModule.getString("createdDate"));
						_modules.setResult("Success");
						_modulesDetaillist.add(_modules);
					}
				}
