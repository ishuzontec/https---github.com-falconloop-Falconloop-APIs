...
public class CustomerDetail {
	...
	public String Error;
	public ArrayList<ApplicantKYC> ApplicantKYCDetails;

	private void setApplicantKYCDetails(ArrayList<ApplicantKYC> ApplicantKYCDetails) {
		this.ApplicantKYCDetails = ApplicantKYCDetails;
	}

	private ArrayList<ApplicantKYC> getApplicantKYCDetails() {
		return ApplicantKYCDetails;
	}
		...
		if (_CustomerDetail.CustomerId <= 0) {
			try {
				if (_Connection != null) {
					...
					ResultSet _ResultSetCustomerEmail = _MYSQLHelper.GetResultSet(
							"SELECT Email FROM customer where Email='" + _CustomerDetail.Email + "'", _Connection);
					if (!_ResultSetCustomerEmail.next()) {

						ResultSet _ResultSet = _MYSQLHelper.GetResultSet(
								"SELECT email FROM company where Company_Id='" + _CustomerDetail.CompanyId + "'",
								_Connection);
						if (_ResultSet.next()) {
							ResultSet _ResultSetCountry = _MYSQLHelper
									.GetResultSet("SELECT country_id FROM country where country_id='"
											+ _CustomerDetail.CountryId + "'", _Connection);
							if (_ResultSetCountry.next()) {
								...
								// _PreparedStatement.setString(16,
								// _CustomerDetail.ActivationCode);
								...
								ResultSet _ResultSetld = _MYSQLHelper.GetResultSet(
										"SELECT MAX(customer_Id) AS customer_Id FROM customer", _Connection);
			...
			try {
				if (_Connection != null) {
					...
					ResultSet _ResultSetCustomer = _MYSQLHelper.GetResultSet(
							"SELECT customer_Id FROM customer where customer_Id='" + _CustomerDetail.CustomerId + "'",
							_Connection);
						...
						if (_ResultSet.next()) {
							...
							if (_ResultSetCountry.next()) {
								...
								if (strpassword != "" | strpassword.length() != 0) {
									String _hashPassword = byteArrayToHexString(
											CompanyDetail.computeHash(_CustomerDetail.Password));
								...
								String sInsertStatement = "UPDATE customer SET 	Company_Id = ?,AccountNumber = ? ,FirstName = ? ,LastName = ? ,Address1 = ? "
										...
										+ ",DOB = ?,ProfileImage = ?,IsAccountActivated = ?,IsActive = ?,CreatedDate = ?,Title= ?,gender=?,building_number= ?,street= ?,town=?,FileName=?,FileType=?,Side=?"
										+ " WHERE customer_Id = ?";
								_PreparedStatement = _Connection.prepareStatement(sInsertStatement);
								_PreparedStatement.setInt(1, _CustomerDetail.CompanyId);
								_PreparedStatement.setString(2, _CustomerDetail.AccountNumber);
								...
								// _PreparedStatement.setString(15,
								// _CustomerDetail.ActivationCode);
	...
	public static ArrayList<CustomerDetail> getCustomerDetails(int CompanyId) {
		...
		try {
...

			{
				...
				ResultSet _ResultSet = _MYSQLHelper.GetResultSet("SELECT * FROM customer where IsDeleted=0 ",
						_Connection);
				// if(_ResultSet.next())
				// {
				while (_ResultSet.next()) {
					...
					_CustomerDetail.ApplicantKYCDetails = _ApplicantKYC.getKYCDetilasApplicant(0, _CustomerDetail.CustomerId, "");
				...
				ResultSet _ResultSet = _MYSQLHelper.GetResultSet(
						"SELECT * FROM customer where Company_Id='" + CompanyId + "' and IsDeleted=0", _Connection);
	...
	public ArrayList<CustomerDetail> getCustomerDetail(int CustomerId) {
		...
		try {
			...
			if (_Connection != null) {
				...
				MYSQLHelper _MYSQLHelper = new MYSQLHelper();
				ResultSet _ResultSet = _MYSQLHelper.GetResultSet(
						"SELECT * FROM customer where customer_Id='" + CustomerId + "'", _Connection);
				if (_ResultSet.next()) {
					CustomerDetail _CustomerDetail=new CustomerDetail();
					_CustomerDetail.setCustomerId(_ResultSet.getInt("customer_Id"));
					_CustomerDetail.setCompanyId(_ResultSet.getInt("Company_Id"));
	...
	public CustomerDetail loginCustomer(CustomerDetail _CustomerDetail) {
		...
		try {
			if (_Connection != null) {
				...
				MYSQLHelper _MYSQLHelper = new MYSQLHelper();
				_CustomerDetail.setPassword(_hashPassword);
				ResultSet _ResultSet = _MYSQLHelper.GetResultSet("SELECT * FROM customer where Email='"
						+ _CustomerDetail.Email + "' and Password ='" + _CustomerDetail.Password + "'", _Connection);
				if (_ResultSet.next()) {
					_CustomerDetail.setCustomerId(_ResultSet.getInt("customer_Id"));
					_CustomerDetail.setCompanyId(_ResultSet.getInt("Company_Id"));
					_CustomerDetail.setAccountNumber(_ResultSet.getString("AccountNumber"));
					...
					ResultSet _ResultSetMerchantKey = _MYSQLHelper.GetResultSet("SELECT * FROM merchantkey where Company_Id='"
							+ _CustomerDetail.CompanyId + "'", _Connection);
					if (_ResultSetMerchantKey.next()) {
						_CustomerDetail.setToken(_ResultSetMerchantKey.getString("TestToken"));
					}
		...
		try {
			if (_Connection != null) {
				...
				if (_ResultSet.next()) {
					...
					String sInsertStatement = "UPDATE customer SET 	IsDeleted = ? " + ",DeletedDate = ? "
							+ " WHERE customer_Id = ?";
					_PreparedStatement = _Connection.prepareStatement(sInsertStatement);
